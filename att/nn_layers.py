...
class BilinearAttentionWeightLayer(lasagne.layers.MergeLayer):
    """
        A bilinear attention layer.
        incomings[0]: batch x len x h
        incomings[1]: batch x h
    ...
    def __init__(self, incomings, num_units,
        ...
        if mask_input is not None:
            incomings.append(mask_input)
        super(BilinearAttentionWeightLayer, self).__init__(incomings, **kwargs)
        self.num_units = num_units
        self.W = self.add_param(init, (self.num_units, self.num_units), name='W_bilinear')
...
class DotLayer(lasagne.layers.MergeLayer):
    ...
    def get_output_for(self, inputs, **kwargs):
        ...
        alpha = T.nnet.softmax(alpha) #batch * len
#        alpha = ((alpha - alpha.min(axis=1).dimshuffle(0, 'x'))/(
#            alpha.ptp(axis=1).dimshuffle(0, 'x')+0.001)) + 0.1
        return alpha
    
    def get_output_shape_for(self, input_shapes):
        return input_shapes[0][:2]	
		
class WeightedAverageLayer(lasagne.layers.MergeLayer):
    """
        weighted mean
        inputs[0]: l_emb1, batch x word_num x hidder_size
        inputs[1]: l_weight, batch x word_num x 1
        inputs[2]: l_mask1, batch x word_num
    """
    def get_output_for(self, inputs, **kwargs):
        alpha = T.reshape(inputs[1], (inputs[1].shape[0], -1))
#        alpha =  T.nnet.softmax(alpha)
        alpha = alpha * inputs[2]
        alpha = alpha / (alpha.sum(axis=1).reshape((alpha.shape[0], 1)))
        pass_e = T.sum(inputs[0] * alpha.dimshuffle(0, 1, 'x'), axis=1)
        return pass_e
    
    def get_output_shape_for(self, input_shapes):
        return (input_shapes[0][0], input_shapes[0][2])
				
class WeightedNormLayer(lasagne.layers.Layer):
    """
        inputs[0]: l_weight, batch x word_num x 1
    """
    def get_output_for(self, inputs, **kwargs):
        alpha = T.reshape(inputs, (inputs.shape[0], -1))
        alpha = ((alpha - alpha.min(axis=1).dimshuffle(0, 'x'))/(
            alpha.ptp(axis=1).dimshuffle(0, 'x')+0.001))
